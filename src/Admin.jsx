...
//   const [phone, setphone] = useState('');
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch('https://rz4gggsw-4522.inc1.devtunnels.ms/v1/admin/login', {
        method: 'POST',
        headers: {
      ...
      if (data.data.Token) {
        ...
        window.location.href = '/AdminPanel'; // Assuming 'Home.jsx' is rendered at '/' route
        setErrorMessage('');
      } 
      // else {
      ...
      <div className="container-fluid page-header" style={{ marginBottom: '50px' }}>
        <div className="container">
          <div className="d-flex flex-column justify-content-center" style={{ minHeight: '200px' }}>
            <h3 className="display-4 text-white text-uppercase">Admin Login</h3>
            <div className="d-inline-flex text-white">
            <p className="m-0 text-uppercase"><Link to='/' className="text-white">Home</Link></p>
              <i className="fa fa-angle-double-right pt-1 px-3" />
              <p className="m-0 text-uppercase">Admin Login</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="container-fluid">
        <div className="container">
          <div className="text-center mb-4">
            ...
            <h4 className=" text-uppercase mb-3" style={{ letterSpacing: '5px' }}>Login To Your Admin Account</h4>
          </div>
